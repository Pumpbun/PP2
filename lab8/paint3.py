...
def main():
    ...
    while True:
        ...
        screen.fill((0, 0, 0))
        if mode == 'rectangle':
            drawRectangle(screen, points, eraser_mode)
        elif mode == 'circle':
            drawCircle(screen, points, eraser_mode)
        else:
            i = 0
            while i < len(points) - 1:
                drawLineBetween(screen, i, points[i], points[i + 1], radius, mode, eraser_mode)
                i += 1
...
def drawLineBetween(screen, index, start, end, width, color_mode, eraser_mode):
    if eraser_mode:
...
def drawRectangle(screen, points, eraser_mode):
    if len(points) > 1:
        if eraser_mode:
            color = (0, 0, 0)  # Use black color to erase
        else:
...
def drawCircle(screen, points, eraser_mode):
    if len(points) > 1:
        if eraser_mode:
            color = (0, 0, 0)  # Use black color to erase
        else:
